...
def check_cube_set(bag: dict, cube_set: str) -> bool:
    cubes = cube_set.split(", ")
    for cube in cubes:
        cube = cube.split(" ")
        cube_count = int(cube[0])
        cube_color = cube[1]
        if cube_count > bag.get(cube_color, 0):
            bag[cube_color] = cube_count
    return bag
...
def check_game(line: str) -> int:
    ...
    cube_sets = game_info[1].split("; ")
    for cube_set in cube_sets:
        bag = check_cube_set(bag, cube_set)
    power = 1
    for key in bag.keys():
        power *= bag[key]
    return power
