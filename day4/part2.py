...
NUMBERS_WON_BY_CARD = [1 for _ in range(cards)]


def distribute_won_cards(card_number: int, winning_numbers: int) -> None:
    next_card = card_number + 1
    while next_card < cards and winning_numbers > 0:
        NUMBERS_WON_BY_CARD[next_card] += NUMBERS_WON_BY_CARD[card_number]
        winning_numbers -= 1
        next_card += 1
    if winning_numbers > 0:
        NUMBERS_WON_BY_CARD[-1] += winning_numbers
...
for card_number, line in enumerate(lines):
    ...
    my_winning_numbers = len([num for num in my_numbers if num in winning_numbers])
    if my_winning_numbers > 0:
        distribute_won_cards(card_number, my_winning_numbers)

print(sum(NUMBERS_WON_BY_CARD))
